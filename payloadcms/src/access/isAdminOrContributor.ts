...
export const isAdminOrContributor = (siteIDFieldName: string = 'profile'): Access => ({ req: { user } }) => {
  ...
  if (user) {
    ...
    if (checkUserRoles(['contributor'], user) && user.profiles?.length > 0) {
      // if (user.roles.includes('contributor') && user.profiles?.length > 0) {
      console.log('contributor profile ', user.profiles)
